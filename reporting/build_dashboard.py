...
def compute_spend_metrics(deals, monthly_spend):
    """
    Compute spend metrics per sub-category (total spend, CAC, cost per SQL, ROI).
    Returns dict of sub_category -> metrics.
    Also returns monthly spend totals for cohort overlay.
    """
    from collections import defaultdict

    # Aggregate total spend per sub-category across all months
    ...
    for sc, spend in total_spend_by_subcat.items():
        ...
        cac = (spend / won) if won > 0 else None
        # ROI: (MRR * 12 - spend) / spend — assumes won_rev is MRR
        arr_from_wins = won_rev * 12
        roi = ((arr_from_wins - spend) / spend * 100) if spend > 0 else None
    ...
    # Monthly spend totals (for cohort table)
    monthly_totals = {}
    for month, subcats in monthly_spend.items():
        monthly_totals[month] = sum(subcats.values())

    return result, monthly_totals


# Raw API result files — v2 with stage entry dates (hs_v2_date_entered_*)
RAW_FILES_V2 = [
    "/Users/mgottron/.claude/projects/-Users-mgottron-Claude-Code/527ec8aa-2cac-47b3-8bc8-1eefdc3a1687/tool-results/mcp-4015ff8e-8996-4ec0-b434-76a999e3f7b7-search_crm_objects-1771011887971.txt",
    "/Users/mgottron/.claude/projects/-Users-mgottron-Claude-Code/527ec8aa-2cac-47b3-8bc8-1eefdc3a1687/tool-results/mcp-4015ff8e-8996-4ec0-b434-76a999e3f7b7-search_crm_objects-1771011896864.txt",
    "/Users/mgottron/.claude/projects/-Users-mgottron-Claude-Code/527ec8aa-2cac-47b3-8bc8-1eefdc3a1687/tool-results/mcp-4015ff8e-8996-4ec0-b434-76a999e3f7b7-search_crm_objects-1771011903860.txt",
    "/Users/mgottron/.claude/projects/-Users-mgottron-Claude-Code/527ec8aa-2cac-47b3-8bc8-1eefdc3a1687/tool-results/mcp-4015ff8e-8996-4ec0-b434-76a999e3f7b7-search_crm_objects-1771011911484.txt",
]
...
OUTPUT_HTML = "/Users/mgottron/Claude Code/dataships_conversion_dashboard.html"
...
def main():
    ...
    spend_metrics, monthly_spend_totals = compute_spend_metrics(deals, monthly_spend)
        ...
        for name, funnel in sorted(funnels_dict.items(), key=lambda x: -len([d for f in x[1] for d in [f] if f["stage"] == "Demo Scheduled (SQL)"])):
            ...
            if has_spend:
                ...
                if sm:
                    ...
                    spend_cells += "<td class='na-cell'>N/A</td>" * 4
    ...
    # Cohort table
    def cohort_table_html(cohort_data, monthly_spend_totals=None):
        ...
        for month, data in cohort_data.items():
            ...
            if has_spend:
                ...
                if month_spend is not None and month_spend > 0:
                    ...
                    # CAC for this cohort = spend / won (if won > 0)
                    if data['won'] > 0:
...
<div class="section">
    ...
    <br>Won $ is MRR. ROI = (ARR from wins − spend) / spend. CAC = total spend / customers won.</div>
    <div class="scroll-x">
"""
    ...
    if spend_metrics:
        ...
        total_cac_all = (total_spend_all / total_won_all) if total_won_all > 0 else None
        total_roi_all = ((total_arr_all - total_spend_all) / total_spend_all * 100) if total_spend_all > 0 else None
...
<div class="section">
    ...
    <div class="section-desc">
        Marketing spend mapped to deal sub-categories. Won $ is MRR; ARR = MRR × 12.
        <br><strong>CAC</strong> = total channel spend / customers won. <strong>ROI</strong> = (ARR from wins − spend) / spend.
    ...
    <div class="callout" style="margin-top: 16px;">
        <strong>How to read ROI:</strong> +100% means you earned 2x what you spent (ARR basis, year 1).
        A positive ROI means the channel pays for itself in year 1 from ARR alone; negative means it takes longer than 12 months to recoup.
        <br>LinkedIn+Vouchers/Tremendous+Meta are grouped as "Paid Campaign". Spend includes all tracked programs (excl. contractors, swag, experiments).
    </div>
</div>
"""
...
<div class="section">
    ...
    <div class="scroll-x">
"""
    html += cohort_table_html(cohort_data, monthly_spend_totals=monthly_spend_totals)
    html += """
    </div>
    <div class="callout" style="margin-top: 16px;">
        ...
        <strong>Cohort CAC</strong> = that month's spend / won deals from that cohort. N/A where spend data is not available.
    </div>
</div>
"""
